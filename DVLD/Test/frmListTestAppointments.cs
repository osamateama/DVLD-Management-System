    ...
    public partial class frmListTestAppointments : Form
    {
        ...
        private void _LoadTestTypeImageAndTitle()
        {
            switch (_TestType)
            {
                ...
                case clsTestType.enTestType.StreeTest:
                    {
        ...
        private void frmListTestAppointments_Load(object sender, EventArgs e)
        {
            ...
            if (dgvLicenseTestAppointments.Rows.Count > 0)
            {
                ...
                foreach (DataGridViewRow row in dgvLicenseTestAppointments.Rows)
                {
        ...
        private void btnAddAppointment_Click(object sender, EventArgs e)
        {
            ...
            if (LastTest.TestResult == false)
            {
                if (!LastTest.TestAppointmentInfo.IsLocked)
                {
                    MessageBox.Show("You must lock the current failed test appointment before scheduling a new one.", "Cannot schedule", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
